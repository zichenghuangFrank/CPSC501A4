...
int main(int argc, char **argv){
    ...
    for(int m=0, n=0, p=0, q=0;n<count1;m+=2,n++,p+=2,q++){
    ...
    for(int m=2*count1, n=2*count2; n<dlength; m++, n++){
        if(m < dlength){
    ...
    for(int i=0; i<dlength; i+=2){
        complexOutput[i] = complexDataArr1[i]*complexDataArr2[i]-complexDataArr1[i+1]*complexDataArr2[i+1];
        complexOutput[i+1] = complexDataArr1[i]*complexDataArr2[i+1]+complexDataArr1[i+1]*complexDataArr2[i];
    }
    ...
    for(int j=0, h=0; j<length; j++, h+=2) {
    ...
    for(int j=0, h=0; j<outputFileSize; j++, h+=2){
    ...
    printf("Algorithm-Based Optimization program used %lf seconds.\n", elapsed / CLOCKS_PER_SEC);
